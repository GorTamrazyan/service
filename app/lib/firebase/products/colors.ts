...
export const firestoreToColor = (doc: DocumentSnapshot<DocumentData>): Color | null => {
...
export const colorToFirestore = (color: Omit<Color, 'id'>): DocumentData => {
  return {
    ...
    createdAt: color.createdAt ? Timestamp.fromDate(color.createdAt) : Timestamp.now(),
    updatedAt: color.updatedAt ? Timestamp.fromDate(color.updatedAt) : Timestamp.now()
  };
};
