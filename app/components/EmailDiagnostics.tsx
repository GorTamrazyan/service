...
export default function EmailDiagnostics({ user }: EmailDiagnosticsProps) {
    ...
    const testEmailSending = async () => {
        ...
        try {
            ...
            addResult(`🔍 Тестирование отправки email для: ${currentUser.email}`);
            addResult(`📧 Email верифицирован: ${currentUser.emailVerified ? 'Да' : 'Нет'}`);

            // Проверяем настройки Firebase
            addResult("🔧 Проверка настроек Firebase...");
            addResult(`🆔 UID пользователя: ${currentUser.uid}`);
            addResult(`📱 Провайдеры: ${currentUser.providerData.map(p => p.providerId).join(', ')}`);
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
            <div className="mb-4">
                ...
                <p className="text-gray-600 text-sm">
                    Этот инструмент поможет определить проблемы с отправкой email для верификации.
                </p>
            </div>
            ...
            {results.length > 0 && (
                <div className="bg-gray-50 rounded-lg p-4 max-h-80 overflow-y-auto">
                    <h4 className="font-semibold text-gray-900 mb-3">📊 Результаты диагностики:</h4>
            ...
            <div className="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <h4 className="font-semibold text-yellow-800 mb-2">💡 Частые причины проблем:</h4>
