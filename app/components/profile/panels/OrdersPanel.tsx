...
export default function OrdersPanel() {
    ...
    const getStatusText = (status: string) => {
        switch (status) {
            case 'pending': return <T>Pending</T>;
            case 'confirmed': return <T>Confirmed</T>;
            case 'shipped': return <T>Shipped</T>;
            case 'delivered': return <T>Delivered</T>;
            case 'cancelled': return <T>Cancelled</T>;
            ...
            <div className="bg-[var(--color-background)] rounded-2xl shadow-xl border border-[var(--color-text)]/10 overflow-hidden">
                ...
                {loading ? (
                    <div className="p-16 text-center">
                        ...
                        <p className="text-[var(--color-text)]/70 mt-4"><T>Loading orders...</T></p>
                    </div>
                ) : orders.length === 0 ? (
                    <div className="p-16 text-center">
                            ...
                            <div key={order.id} className="p-6">
                                ...
                                <div className="bg-[var(--color-secondary)]/20 rounded-lg p-4">
                                    <div className="flex items-center justify-between mb-3">
                                        <h5 className="font-medium text-[var(--color-primary)]">
                                            <T>Order Items</T> ({order.products.length})
                                        </h5>
                                    </div>
                                ...
                                <div className="mt-4 bg-[var(--color-secondary)]/20 rounded-lg p-4">
                                    ...
                                    <p className="text-sm text-[var(--color-text)]/70">
                                        <strong><T>Name:</T></strong> {order.customerInfo.name}
                                    </p>
                                    <p className="text-sm text-[var(--color-text)]/70">
                                        <strong><T>Email:</T></strong> {order.customerInfo.email}
                                    </p>
                                    {order.customerInfo.phone && (
                                        <p className="text-sm text-[var(--color-text)]/70">
                                            <strong><T>Phone:</T></strong> {order.customerInfo.phone}
                                        </p>
                                    )}
                                    <p className="text-sm text-[var(--color-text)]/70">
                                        <strong><T>Address:</T></strong> {order.customerInfo.address}
                                    </p>
                                </div>
                            </div>
                        ))}
